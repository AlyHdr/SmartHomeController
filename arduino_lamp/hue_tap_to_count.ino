...
void setup() {
  ...
  pinMode(BUTTON_PIN_COUNT, INPUT);
...
void loop() {
  ...
  if ( (t - t0) >= DELTA_T ) {
    ...
    btn_count_state = digitalRead(BUTTON_PIN_COUNT); 
    ...
    if(btn_count_state == HIGH){
      id_counter++;
      Serial.print("Clicked ...  counter value: ");
    ...
    if(buttonState == HIGH){
          ...
          int temp_count = init_count + id_counter;
          LIGHT_ID = String(temp_count);
...
char spring_server [] = "http://alyhdr.cleverapps.io/";
void test_http(){
    Serial.println("Trying to turn ligh off");
  if(client.connect(server, 80)){
    Serial.println("connected !");
     client.println("PUT /api/lights/"+LIGHT_ID+"/switch HTTP/1.1");
    client.println("Host: 192.168.1.131");                          
     client.println("Connection: close");
     client.println("Content-Type: application/x-www-form-urlencoded");
